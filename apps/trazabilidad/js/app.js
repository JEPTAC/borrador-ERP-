(function(){
  ...
  var state={session:null,roles:null,catalog:null,group:"user",modules:[],currentModule:"home",cases:[],credits:[],searchTab:"all",searchIndex:0,commandReturnFocus:null};
  ...
  function engineUrl(action){var url=new URL(rootBase+"engine/modules/"+action.module+"/",location.href);url.searchParams.set("route",action.route);url.searchParams.set("embedded","1");url.searchParams.set("nova","1");url.searchParams.set("source","shell");return url.href;}
  function openAction(module,action){if(!allowed(action.groups)){toast("Acceso no autorizado","Su rol no tiene permiso para abrir esta transacción.","error");return;}state.currentModule=module.id;renderSidebar();var ws=U.qs("#novaWorkspace"),frame=U.qs("#workspaceFrame"),loading=U.qs("#workspaceLoading");U.qs("#workspaceTitle").textContent=module.name+" · "+action.name;loading.removeAttribute("hidden");ws.removeAttribute("hidden");frame.title=action.name+" · "+module.name;frame.src=engineUrl(action);frame.onload=function(){loading.setAttribute("hidden","");};history.replaceState(null,"","#action="+encodeURIComponent(module.id+":"+action.id));closeMobileNav();}
  function closeWorkspace(){var ws=U.qs("#novaWorkspace"),frame=U.qs("#workspaceFrame");ws.setAttribute("hidden","");if(frame.src&&frame.src!=="about:blank")frame.src="about:blank";U.qs("#workspaceLoading").setAttribute("hidden","");}
